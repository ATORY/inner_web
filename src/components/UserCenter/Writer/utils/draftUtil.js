...
export function getContentLengthBetween(
  editorState,
  startKey,
  startOffset,
  endKey,
...
export function getSelection(editorState) {
  ...
  const editorWiseOffset = getContentLengthOfAllBlocksBefore(
    editorState,
    startKey
  )
  const offsetBetweenKeys = getContentLengthBetween(
    editorState,
    startKey,
    startOffset,
    endKey,
...
const findBlockKeyAndOffsetForPosition = (
  position,
  block,
  globalOffset,
  blockOffset,
  ...
  if (position > globalOffset + block.getText().length) {
    ...
    return findBlockKeyAndOffsetForPosition(
      position,
      contentState.getBlockAfter(block.getKey()),
      globalOffset + block.getText().length + 1,
      0,
      contentState
    )
  }
...
export function buildSelectionState(contentState, selection) {
  const firstBlock = contentState.getFirstBlock()
  if (firstBlock === null) {
    return null
  }
  const startBlockData = findBlockKeyAndOffsetForPosition(
    selection.start,
    firstBlock,
    0,
    0,
  ...
  const endBlockData = findBlockKeyAndOffsetForPosition(
    selection.end,
    startBlockData.block,
    startBlockData.globalOffset,
    startBlockData.blockOffset,
    contentState
  )
...
export function getMarkdownStateFromDraftState(editorState) {
  return {
...
export function buildNewDraftState(currentState, markdownState) {
